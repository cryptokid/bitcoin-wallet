...
public final class SendCoinsFragment extends SherlockFragment
{
	...
	private void handleGo()
	{
		...
		new SendCoinsOfflineTask(wallet, backgroundHandler)
		{
			...
			{
				if (paymentIntent != null && directPaymentEnableView.isChecked())
				{
					final DirectPaymentTask.ResultCallback callback = new DirectPaymentTask.ResultCallback()
					{
						...
						public void onFail(final String message)
						{
							...
							dialog.setTitle("Direct transaction failed");
					...
					{
						new DirectPaymentTask.HttpPaymentTask(backgroundHandler, callback, paymentIntent.paymentUrl).send(paymentIntent.bip,
					...
					else if (paymentIntent.isBluetoothPaymentUrl() && bluetoothAdapter != null && bluetoothAdapter.isEnabled())
					{
						new DirectPaymentTask.BluetoothPaymentTask(backgroundHandler, callback, bluetoothAdapter, paymentIntent.getBluetoothMac())
								.send(paymentIntent.bip, transaction, changeAddress, amount, paymentIntent.payeeData);
					}
				}
			}
	...
	private void updateView()
	{
		...
		if (paymentIntent != null && paymentIntent.hasPaymentUrl())
		{
