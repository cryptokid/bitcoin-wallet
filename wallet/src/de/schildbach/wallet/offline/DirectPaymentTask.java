...
public abstract class DirectPaymentTask
{
	...
	public final static class HttpPaymentTask extends DirectPaymentTask
	{
		...
		@Override
		public void send(@Nonnull final PaymentIntent.BIP bip, @Nonnull final Transaction transaction, @Nonnull final Address refundAddress,
				@Nonnull final BigInteger refundAmount, @Nonnull final byte[] merchantData)
		{
			super.backgroundHandler.post(new Runnable()
			{
				...
				public void run()
				{
					if (!bip.equals(PaymentIntent.BIP.BIP70))
	...
	public final static class BluetoothPaymentTask extends DirectPaymentTask
	{
				...
				@Nonnull final BigInteger refundAmount, @Nonnull final byte[] merchantData)
		{
			super.backgroundHandler.post(new Runnable()
			{
				...
				public void run()
				{
					log.info("trying to send tx {} via bluetooth {} using {} protocol", new Object[] { transaction.getHashAsString(), bluetoothMac,
							bip });
	...
	public abstract void send(@Nonnull PaymentIntent.BIP bip, @Nonnull Transaction transaction, @Nonnull Address refundAddress,
